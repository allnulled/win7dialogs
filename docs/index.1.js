...
this.root.$window.win7dialogs.open( "<p>Un nuevo mensaje en un diálogo!</p><button>Aceptar</button>" );
} catch(error) {
console.log(error);
throw error;
}
...
const formulario = (await this.root.$window.win7dialogs.open( "" + "<div>" + "  <div style='padding:8px;'>" + "    <div>Usuario: </div>" + "    <input style='width:100%;' type='text' name='nombre' placeholder='usuario' value='admin' />" + "  </div>" + "  <div style='padding:8px;'>" + "    <div>Contraseña: </div>" + "    <input style='width:100%;' type='password' name='contrasenya' placeholder='contraseña' value='admin' />" + "  </div>" + "  <div style='padding:8px; text-align:right;'>" + "    <button>Entrar</button>" + "  </div>" + "</div>",
"Fake login" ));
const [ elemento_nombre, elemento_contrasenya ] = formulario.elements;
const nombre = elemento_nombre.value;
const contrasenya = elemento_contrasenya.value;
const formulario2 = (await this.root.$window.win7dialogs.open( "" + "<div>" + "  <div style='padding:8px;'>" + "    <div>La ventana anterior nos ha proporcionado los datos. Ahora podemos reusarlos en otra ventana aparte.</div>" + "  </div>" + "  <div style='padding:8px;'>" + "    <div>¿A que su usuario era " + ( JSON.stringify(nombre, null, 2) ) + " y su contraseña era " + ( JSON.stringify(contrasenya, null, 2) ) + "?</div>" + "  </div>" + "  <div style='padding:8px; text-align:right;'>" + "    <button>Sí. Lo acepto.</button>" + "  </div>" + "</div>",
